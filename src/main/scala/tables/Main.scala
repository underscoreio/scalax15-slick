...
object Main {
  ...
  class AlbumTable(tag: Tag) extends Table[Album](tag, "messages") {
    ...
    def * = (artist, title, released, rating, id) <> (Album.tupled, Album.unapply)
  }
  ...
  val selectAlbumsAction =
    AlbumTable.result.map(_ map (_.toString))
  ...
  def main(args: Array[String]): Unit =
    exec(createTablesAction andThen insertAlbumsAction andThen selectAlbumsAction).foreach(println)
